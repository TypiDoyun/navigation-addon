export const getDistanceSquared = (first, second) => {
...
export const getSegmentDistance = (line, vertex) => {
    return getSegmentDistanceSquare(line, vertex) ** 0.5;
};
export const getSegmentDistanceSquare = (line, vertex, lineCut = 0) => {
    const lineDirection = {
        x: line[1].x - line[0].x,
        y: line[1].y - line[0].y,
        z: line[1].z - line[0].z,
    ...
    if (lineLength === 0)
        return getDistanceSquared(line[0], vertex);
    ...
    if (projection < 0)
        ...
        return getDistanceSquared(line[1], vertex);
    else {
