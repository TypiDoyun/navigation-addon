...
export const removeEdge = (eventData) => {
    ...
    if (!node)
        return sendMessage(player, "해당 블록에는 노드가 존재하지 않습니다.", "c");
    if (firstNode) {
        ...
        if (!firstNode.connections.includes(node) || !node.connections.includes(firstNode))
            return sendMessage(player, "두 노드를 잇는 간선이 없습니다.");
        if (firstNode.connections.includes(node))
            firstNode.connections.splice(firstNode.connections.indexOf(node), 1);
        if (node.connections.includes(firstNode))
...
export const cancelRemove = (eventData) => {
